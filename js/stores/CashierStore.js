...
/**
 * @type {{fees: {enableBP: number, cashType: string, enableCash: number, enableFree: number, structure: Array}, displayName: string, bonus: Array, waitLimits: boolean, limitCurrency: Array, Name: string, processorClass: number, skins: Array, processorId: number, limitRules: Array, load(*): void, rate: number, processorIdParent: number, limits: Array}}
 * @private
 */
let _processor = {
  processorClass: 0,
  ...
  load(processorId){
    ...
    //set values
    this.processorIdParent = processor.processorIdParent
...
let CashierStore = assign({}, EventEmitter.prototype, {
   ...
   *
   * @returns {{fees: {enableBP: number, cashType: string, enableCash: number, enableFree: number, structure: Array}, displayName: string, bonus: Array, waitLimits: boolean, limitCurrency: Array, Name: string, processorClass: number, skins: Array, processorId: number, limitRules: Array, load, (*): void, rate: number, processorIdParent: number, limits: Array}}
   */
  getProcessor: () => {
    return _processor
  },
    ...
    try{
      switch(action){
        ...
        case actions.PROCESSORS_RESPONSE:
          ...
          const depositProcessors = data.response.processors.deposit || []
          if(depositProcessors.length){
            depositProcessors.map((processor, i) => {
              if(processor.skinList && processor.SkinsOnly == 1){
                processor.skinList.forEach((skin) => {
                  _customer.depositProcessors.push({
                    Name: skin.DisplayName,
                    NameParent: processor.Name,
                    DisplayName: skin.DisplayName,
                    ClassName: processor.ClassName,
                    caProcessor_Id: skin.caProcessorSkin_Id,
                    processorIdParent: processor.caProcessor_Id,
                    ClassDisplayName: processor.ClassDisplayName,
                    caProcessorClass_Id: processor.caProcessorClass_Id
                  })
                })
              }else{
                _customer.depositProcessors.push(processor)
              }
            })
          }

          const withdrawProcessors = data.response.processors.withdraw || []
          if(withdrawProcessors.length){
            withdrawProcessors.map((processor, i) => {
              if(processor.skinList && processor.SkinsOnly == 1){
                processor.skinList.forEach((skin) => {
                  _customer.withdrawProcessors.push({
                    Name: skin.DisplayName,
                    NameParent: processor.Name,
                    DisplayName: skin.DisplayName,
                    ClassName: processor.ClassName,
                ...
                _customer.withdrawProcessors.push(processor)
              }
            })
          }
