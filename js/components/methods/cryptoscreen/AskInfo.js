...
let AskInfo = React.createClass({
	...
	buildCurrenciesContainer(){
		...
		orderCurrencies[1] = 'BTC';
		orderCurrencies[2] = 'BCH';
		orderCurrencies[3] = 'ETH';
		orderCurrencies[4] = 'LTC';
		orderCurrencies[4] = 'XMR';
		orderCurrencies[5] = 'DASH';

		if(currencies){
			currency = Object.keys(currencies);
			availableCurrencies = currency.filter(function(current){
				return ((current != 'XRP') && currencies[current].status == 'available');
			});

			unavailableCurrencies = currency.filter(function(current){
				if((current != 'XRP') && currencies[current].status != 'available'){
	...
	render(){
		...
		return (
			<div id="cryptoScreenAskInfo" className="box">
				<div className="row">
