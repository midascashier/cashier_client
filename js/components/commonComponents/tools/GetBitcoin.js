	...
	render(){
		...
		return (
			<div className="col-sm-12 mod-center">
				<button className="btn-getBitcoin coinDirectElement" type="button" value="Get bitcoins"
								onClick={this.showBitcoinContent}>
				...
				<div id='getBitcoinModal' className="modal">
					<div id='getBitcoinModal-content'>
						...
						<div id="getBitcoinModal-Form">
							...
							<input type="password" id="coinDirectConfirmPass"
										 placeholder={translate(placeConfirmPass)}/>
							<button id="coinDirectGo" className="btn btn-green"
											onClick={this.login}>{translate(this.state.btnGO)}</button>
							<p>{translate(this.state.messageGO)}<a onClick={this.switchAction}>{translate('HERE')}</a>
							</p>
						</div>
					</div>
				</div>
